...
void CAN_IC_READ_REGISTER_2(uint8_t address, uint8_t* buffer)
{
	...
	HAL_GPIO_WritePin(CAN2_CS_GPIO_Port, CAN2_CS_Pin, GPIO_PIN_RESET); // Initialize instruction by setting CS pin low
	HAL_SPI_Transmit(&hspi4, packet, 2, 100U); //transmit
	HAL_SPI_Receive(&hspi4, buffer, 1, 100U); //receive register contents
	HAL_GPIO_WritePin(CAN2_CS_GPIO_Port, CAN2_CS_Pin, GPIO_PIN_SET); // Terminate instruction by setting CS pin high
}
...
void CAN_IC_WRITE_REGISTER_2(uint8_t address, uint8_t value)
{
	...
	HAL_GPIO_WritePin(CAN2_CS_GPIO_Port, CAN2_CS_Pin, GPIO_PIN_RESET); //set CS pin low
	HAL_SPI_Transmit(&hspi4, packet, 3, 100U);	//transmit
	HAL_GPIO_WritePin(CAN2_CS_GPIO_Port, CAN2_CS_Pin, GPIO_PIN_SET); //set CS pin high
}
...
void CAN_IC_WRITE_REGISTER_BITWISE_2(uint8_t address, uint8_t mask, uint8_t value)
{
	...
	HAL_GPIO_WritePin(CAN2_CS_GPIO_Port, CAN2_CS_Pin, GPIO_PIN_RESET); //set CS pin low
	HAL_SPI_Transmit(&hspi4, packet, 4, 100U); //transmit
	HAL_GPIO_WritePin(CAN2_CS_GPIO_Port, CAN2_CS_Pin, GPIO_PIN_SET); //set CS pin high
}
...
void ConfigureCANSPI_2(void)
{
	...
	HAL_SPI_Transmit(&hspi4, &resetCommand, 1, 100U);  //reset IC to default
	HAL_GPIO_WritePin(CAN2_CS_GPIO_Port, CAN2_CS_Pin, GPIO_PIN_SET);

	CAN_IC_WRITE_REGISTER_2(0x0f, 0x80); //Ensure IC is in configuration mode

	CAN_IC_WRITE_REGISTER_2(CNF1, CONFIG_CNF1); //configure CNF1
	CAN_IC_WRITE_REGISTER_2(CNF2, CONFIG_CNF2); //configure CNF2
	CAN_IC_WRITE_REGISTER_2(CNF3, CONFIG_CNF3); //configure CNF3

	CAN_IC_WRITE_REGISTER_2(CANINTE, 0xff); 	//configure interrupts, currently enable error and and wakeup INT
	CAN_IC_WRITE_REGISTER_2(CANINTF, 0x00); 	//clear INTE flags
									   		//this should be a bit-wise clear in any other case to avoid unintentionally clearing flags

	CAN_IC_WRITE_REGISTER_2(0x0c, 0x0f); //set up RX0BF and RX1BF as interrupt pins

	CAN_IC_WRITE_REGISTER_2(RXB0CTRL, 0x60); //accept any message on buffer 0
	CAN_IC_WRITE_REGISTER_2(RXB1CTRL, 0x60); //accept any message on buffer 1

	CAN_IC_WRITE_REGISTER_2(0x0f, 0x04); //Put IC in normal operation mode with CLKOUT pin enable and 1:1 prescaler

	#if CAN_TEST_SETUP
	CAN_IC_WRITE_REGISTER_2(0x0F, 0x44);	// Put IC in loop-back mode for testing as well as enable CLKOUT pin with 1:1 prescaler
	#endif
}
...
uint8_t checkAvailableTXChannel_2()
{
    ...
    for ever
    {
        ...
        CAN_IC_READ_REGISTER_2(TXB0CTRL, &TXB0Status);
        ...
        CAN_IC_READ_REGISTER_2(TXB1CTRL, &TXB1Status);
        ...
        CAN_IC_READ_REGISTER_2(TXB2CTRL, &TXB2Status);
...
{
	uint8_t channel = checkAvailableTXChannel_2();
	...
	CAN_IC_WRITE_REGISTER_2(initialBufferAddress + 1, TXBNSIDH); // SD 10-3
	CAN_IC_WRITE_REGISTER_2(initialBufferAddress + 2, TXBNSIDL); // SD 2-0
	CAN_IC_WRITE_REGISTER_2(initialBufferAddress + 5, TXBNDLC);  // DLC

	// Set data to registers
	uint8_t initialDataBufferAddress = initialBufferAddress + 6;
	for(int i = 0; i < msg->DLC; i++)
	{
		CAN_IC_WRITE_REGISTER_2(initialDataBufferAddress + i, msg->data[i]); //write to relevant data registers
	}

	// set transmit buffer priority to 3 (max)
	// write to TXBNCTRL<1:0>
	CAN_IC_WRITE_REGISTER_BITWISE_2(initialBufferAddress, 0x03, 0x03);

	HAL_GPIO_WritePin(CAN2_CS_GPIO_Port, CAN2_CS_Pin, GPIO_PIN_RESET);
	HAL_SPI_Transmit(&hspi4, &sendCommand, 1, 100U);  // Send command to transmit
	HAL_GPIO_WritePin(CAN2_CS_GPIO_Port, CAN2_CS_Pin, GPIO_PIN_SET);
}
...
void sendExtendedCANMessage_2(CANMsg *msg)
{
	...
	CAN_IC_WRITE_REGISTER_2(initialBufferAddress + 1, TXBNSIDH); // SD 10-3
	CAN_IC_WRITE_REGISTER_2(initialBufferAddress + 2, TXBNSIDL); // SD 2-0, ED 17-16
	CAN_IC_WRITE_REGISTER_2(initialBufferAddress + 3, TXBNEID8); // ED 15-8
	CAN_IC_WRITE_REGISTER_2(initialBufferAddress + 4, TXBNEID0); // ED 7-0
	CAN_IC_WRITE_REGISTER_2(initialBufferAddress + 5, TXBNDLC);  // DLC
	...
	CAN_IC_WRITE_REGISTER_BITWISE_2(initialBufferAddress, 0x03, 0x03); //set transmit buffer priority to 3 (max)
...
void receiveCANMessage_2(uint8_t channel, uint32_t* ID, uint8_t* DLC, uint8_t* data)
{
	...
	CAN_IC_READ_REGISTER_2(initialBufferAddress + 1, &RXBNSIDH); // SD 10-3
	CAN_IC_READ_REGISTER_2(initialBufferAddress + 2, &RXBNSIDL); //SD 2-0, IDE, ED 17-16
	CAN_IC_READ_REGISTER_2(initialBufferAddress + 5, &RXBDLC);   //DLC
	...
	if(RXBNSIDL & 0x08)	// Check RXBmSIDL.IDE to verify if CAN message has extended identifier
	{
		...
		CAN_IC_READ_REGISTER_2(initialBufferAddress + 3, &RXBNEID8); //ED 15-8
		CAN_IC_READ_REGISTER_2(initialBufferAddress + 4, &RXBNEID0); //ED 7-0
	...
	*DLC = RXBDLC & 0x0F; 
	...
	for(int i = 0; i < *DLC; i++)
	{
		CAN_IC_READ_REGISTER_2(initialDataBufferAddress + i, (data++)); //read from relevant data registers
	}

	CAN_IC_WRITE_REGISTER_BITWISE_2(CANINTF, channel + 1, channel + 1); //clear interrupts
	return;
}
